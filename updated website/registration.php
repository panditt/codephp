...
$q = "SELECT mob FROM myusers WHERE mobileno='$mob'";
...
{
	$query = "INSERT INTO myusers VALUES ('','$n','$em','$mob','$p','$age','$clst','$pur','$add','$gen')";
	mysqli_query($con,$query);
	//mkdir("images/".$_POST['umail']);
	//move_uploaded_file($_FILES['file']['tmp_name'],"images/".$_POST['umail']."/".$_FILES['file']['name']);
	echo "<center><h1 style='color:green'>Details Saved!</h1></center>";
	echo '<script>message("details saved! please come tommorow at the time of class")</script>';
}
}
else
{
...
<option value="health maintain">health maintaining</option></select></p>
