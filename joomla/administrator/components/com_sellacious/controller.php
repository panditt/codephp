...
class SellaciousController extends JControllerLegacy
{
	...
	public function display($cachable = false, $urlparams = false)
	{
		...
		if (class_exists('SellaciousHelper'))
		{
			...
			$path   = $helper->core->getAppPath();
			$app    = JFactory::getApplication();
			$launch = $app->input->post->getInt('redirect');
			$auto   = $app->input->post->getInt('auto_redirect');
			...
			if ($launch || $helper->config->get('auto_redirect_backoffice', 0, 'sellacious', 'application'))
			{
				$app->redirect(JUri::root(true) . '/' . $path);
			}
