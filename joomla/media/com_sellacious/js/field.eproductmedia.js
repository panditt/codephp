	...
	JFormFieldEProductMedia.prototype = {
		...
		updateList: function (files, $cell, clear) {
			...
			$.each(files || [], function (i, file) {
				...
				fileName = fileName.length <= 30 ? fileName : fileName.substring(0, 14) + '&hellip;' + fileName.substring(fileName.length - 13, fileName.length);
		...
		updateRecords: function (records, clear) {
			...
			$.each(records || [], function (i, record) {
				...
				var keys = ['id', 'product_id', 'variant_id', 'seller_uid', 'tags', 'version', 'released', 'is_latest', 'state'];
