...
jQuery(document).ready(function ($) {
	...
	SellaciousBanner.prototype = {
		...
		fetch: function (callback) {
			...
			$.ajax({
					url: dotcom + '/index.php?option=com_jarvis&view=banners&format=json',
					type: 'POST',
					dataType: 'json',
					cache: false,
		...
		display: function (banners) {
			...
			if (banners.length)
				$(banners).each(function (index, banner) {
					banner.link = dotcom + '/index.php?option=com_banners&task=click&id=' + banner.id;
		...
		image: function (banner) {
			...
			if (element && $(element).length) {
				var img = $('<img/>', {
					src: dotcom + '/' + banner.params['imageurl'],
