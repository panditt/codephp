...
/*
			var rows = $($id + ' .sfpprow').length;
			if (rows <= 1) {
				$($id + ' .sfpprow-remove').attr('disabled', 'disabled');
				alert(Joomla.JText._('COM_SELLACIOUS_FLD_GRD_PRODUCTPRICES_LAST_ROW_DELETE_ERROR'));
				return false;
			}
*/
			...
			// hide delete button if last row remaining
			if ($($id + ' .sfpprow').length <= 1) {
				$($id + ' .sfpprow-remove').attr('disabled', 'disabled');
			}
		},
