		...
		init: function (selector, token, controller) {
			...
			*/
			...
			$that.element.on('click', '.edit-address', function () {
				var id = $(this).data('id');
				Joomla.renderMessages({info: ['Please wait while we load the address for editing&hellip;']});
				$that.getAddress(id, function (data) {
					$that.showForm(data);
				});
			});
			...
			$that.element.on('click', '.copy-address', function () {
				Joomla.renderMessages({info: ['Please wait while we load the copy of the address&hellip;']});
