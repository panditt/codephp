...
$params = array(
	'title'    => $this->escape(addslashes($item->title)),
	'url'      => $url_m,
	'height'   => '600',
	'width'    => '800',
	'keyboard' => true,
	...
	<div class="product-box spl-cat-<?php echo (int) $item->spl_listing_catid; ?>" data-rollover="container">
		...
		<div class="product-info-box">
			...
			<?php if ($allow_rating && in_array('products', $rating_pages)): ?>
				<div class="product-stars pull-left"><?php
					echo $this->helper->core->getStars($item->rating, true, 5.0) ?></div>
			<?php endif; ?>
			...
			if ($item->price_display == 0)
			{
				...
				if ($price_display > 0 && in_array('products', $price_d_pages))
				{
					...
					if ($price_display == 2 && round($item->list_price, 3) > 0)
					{
						...
						<div class="product-price"><?php echo $price ?></div><?php
					}
					else
					{
			...
			elseif ($item->price_display == 3 && in_array(3, $allowed_price_display))
			{
				$options = array(
					'title'    => (JText::sprintf('COM_SELLACIOUS_PRODUCT_PRICE_DISPLAY_OPEN_QUERY_FORM_FOR',
						$this->escape(addslashes($item->title)), $this->escape(addslashes($item->variant_title)))),
					'backdrop' => 'static',
					'keyboard' => true,
				...
				<div class="center"><a href="#query-form-<?php echo $item->code ?>"
						role="button" data-toggle="modal" class="btn btn-mini btn-primary"><i
			...
			<?php if ($item->price_display == 0): ?>
				<?php if ((int) $item->stock_capacity > 0): ?>
					<?php if ($allow_checkout && in_array('products', $buynow_pages)): ?>
					...
					<?php if ($allow_checkout && in_array('products', $cart_pages)): ?>
						<button type="button" class="btn btn-mini btn-default btn-add-cart add" data-item="<?php echo $item->code; ?>">
