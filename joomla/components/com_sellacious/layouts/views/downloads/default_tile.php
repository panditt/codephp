...
$l = $delivery->get('attributes.download_period.l', 30) ?: 20;
$p = $delivery->get('attributes.download_period.p', 'day') ?: 'day';

$now        = JFactory::getDate();
$expiry     = JFactory::getDate($delivery->get('order_date'))->modify("+$l $p");
$item_title = trim(sprintf('%s - %s', $delivery->get('product_title'), $delivery->get('variant_title')), '- ');
?>
<div class="w100p tile-box toggle-box">
	...
	<div class="tile-body toggle-element hidden">
		<h4 class="strong"><?php echo $item_title; ?></h4>
		<div class="fieldset">
			<?php
			...
			$expiry_date = JHtml::_('date', $expiry->format('r'), 'D, F d, Y h:i A');
			?>
		...
		<table class="w100p">
			...
			<?php foreach ($delivery->get('items') as $media): ?>
				...
				<tr>
					<td>
						<strong><?php echo $this->escape($media->media->original_name ?: $item_title); ?></strong><br>
