...
class SellaciousModelStore extends SellaciousModelList
{
	...
	protected function getListQuery()
	{
		...
		$counts = $this->isShippableAt();

		if (!$query || $counts === false)
		{
			// Just a fallacy
	...
	protected function getPriceQuery()
	{
		...
		$seller_uid = $this->getState('store.id');
	...
	public function getFilters()
	{
		...
		foreach ($filters as $key => $filter)
		{
			...
			$subQueryP->select('product_id')
				...
				->where('seller_uid = ' . (int) $this->getState('store.id'));
	...
	protected function isShippableAt()
	{
		...
		$seller_uid = (int) $this->getState('store.id');

		$query->select('COUNT(1)')->from('#__sellacious_seller_shippable')
			->where('(gl_id = 0 OR gl_id = ' . implode(' OR gl_id = ', $queried) . ')')
			->where('state = 1')
			->where('seller_uid = ' . (int) $seller_uid);

		$cnt = $this->_db->setQuery($query)->loadResult();
